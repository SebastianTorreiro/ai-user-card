...
export function ChampionCard({
  ...
  const [activeTab, setActiveTab] = useState<"skills" | "lore" | "stats">(
    "skills",
  );
    ...
    <div className="w-full max-w-4xl bg-[#0a0a12] border border-[#c8aa6e] rounded-sm shadow-2xl overflow-hidden font-sans text-[#f0e6d2]">
      <div className="relative h-80 w-full group">
        <Image
          ...
          fill
          unoptimized
          className="object-cover object-[50%_20%] opacity-80 group-hover:opacity-100 transition-opacity duration-700"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a12] via-transparent to-transparent"></div>

        <div className="absolute bottom-0 left-0 p-8 w-full">
          <h2 className="text-[#c8aa6e] font-bold tracking-widest uppercase text-sm mb-1">
            {region} • {charClass}
          </h2>
          <h1 className="text-5xl font-black text-white tracking-tighter uppercase italic drop-shadow-md">
            {name}
          </h1>
          <p className="text-xl text-[#a09b8c] font-serif italic">
            &quot;{title}&quot;
          </p>
        </div>
      </div>
...
function AbilityRow({
  ...
  return (
    <div className="flex gap-4 group">
      ...
      <div>
        <h3
          className={`font-bold uppercase tracking-wide ${isUlt ? "text-[#c8aa6e]" : "text-white"}`}
        >
          {data.name}
        </h3>
        ...
        <p className="text-[#a09b8c] text-sm leading-relaxed">
          {data.description}
        </p>
      </div>
    </div>
